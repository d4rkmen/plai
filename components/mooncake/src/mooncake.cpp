...
static const char* TAG = "MOONCAKE";

using namespace MOONCAKE;

Mooncake::~Mooncake()
{
    ESP_LOGW(TAG, "mooncake destruction");
...
void Mooncake::init()
{
    ESP_LOGI(TAG, "mooncake init :)");

    /* Init user data */
    /* If user data not set */
    if (_user_data == nullptr)
    {
        ESP_LOGI(TAG, "create userdata");
...
void Mooncake::_data_base_setup_internal()
{
    ...
    {
        ESP_LOGI(TAG, "call db setup callback");
